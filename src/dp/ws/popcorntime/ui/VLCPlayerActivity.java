...
public class VLCPlayerActivity extends PlayerBaseActivity implements IVideoPlayer {
	...
	private void startPrepareTorrent() {
		...
		if (isTorrentVideoReady) {
			...
			if (torrentFilePath != null && !"".equals(torrentFilePath)) {
				mTorrent.loadVideo(torrentFilePath, getIntent().getStringExtra(FILENAME_EXTARA_KEY));
			}
		}
	}
	...
	private void seekTorrentToPosition() {
		boolean isHavePiece = mTorrent.seekToPosition(mLibVLC.getLength(), mLibVLC.getTime());
		if (!isHavePiece) {
			pause();
			showOverlay();
		}
	...
	private static class VideoPlayerEventHandler extends WeakHandler<VLCPlayerActivity> {
		...
		public void handleMessage(Message msg) {
			...
			case EventHandler.MediaPlayerEndReached:
				...
				if (TorrentState.FINISHED == activity.mTorrent.getTorrentState()) {
					activity.changeAudioFocus(false);
					activity.endReached();
				} else {
	...
	private void handleHardwareAccelerationError() {
		...
		// AlertDialog.Builder(VLCPlayerActivity.this).setPositiveButton(R.string.ok,
			...
			AlertDialog dialog = new AlertDialog.Builder(VLCPlayerActivity.this).setTitle(R.string.track_audio)
					.setSingleChoiceItems(arrList, listPosition, new DialogInterface.OnClickListener() {
						...
						public void onClick(DialogInterface dialog, int listPosition) {
							...
							MediaDatabase.getInstance().updateMedia(mLocation, MediaDatabase.mediaColumn.MEDIA_AUDIOTRACK, trackID);
    ...
    */
	...
	//
	// AlertDialog dialog = new
	// AlertDialog.Builder(VLCPlayerActivity.this).setTitle(R.string.track_text)
	// .setSingleChoiceItems(arrList, listPosition, new
	// DialogInterface.OnClickListener() {
	// @Override
	// public void onClick(DialogInterface dialog, int listPosition) {
	// int trackID = -2;
	...
	// return;
	//
	// MediaDatabase.getInstance().updateMedia(mLocation,
	// MediaDatabase.mediaColumn.MEDIA_SPUTRACK, trackID);
    ...
    */
	...
	private void updateOverlayPausePlay() {
		...
		if (isCastEnabled) {
			mPlayPause.setBackgroundResource(isCastPlaying ? R.drawable.ic_pause_circle : R.drawable.ic_play_circle);
		} else {
	...
	private int setOverlayProgress() {
		...
		int progress = (int) (sizeMB == 0 ? 0 : (max * progressSize - hackMb) / sizeMB);
